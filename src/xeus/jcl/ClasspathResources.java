...
//            logger.trace( "Attempting to load a remote resource." );
        ...
        if( url.toString().toLowerCase().endsWith( ".jar" )||url.toString().toLowerCase().endsWith( ".zip" ) ) {
            loadJar( url );
            return;
        }
    ...
    private void loadResource(File fol, String packName) throws IOException {
        if( fol.isFile() ) {
            if( fol.getName().toLowerCase().endsWith( ".class" ) ) {
                ...
                if( fol.getName().toLowerCase().endsWith( ".jar" )||fol.getName().toLowerCase().endsWith( ".zip" ) ) {
...
/*            for( String f : fol.list() ) {*/
        	for(int i=0;i<fol.list().length;i++) {
        		String f=fol.list()[i];
