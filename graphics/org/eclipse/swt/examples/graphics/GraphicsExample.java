...
public class GraphicsExample {
	...
	ArrayList tabs_in_order; // stores GraphicsTabs in the order that they
								// appear in the tree
	...
	static final ResourceBundle RESOURCE_BUNDLE = /*
												 ...
												 * "examples_graphics");
												 *///$NON-NLS-1$
	ResourceBundle.getBundle("examples_graphics", new Locale(""), Thread
	...
	void createControls(final Composite parent) {
		...
		data = new FormData();
		data.left = new FormAttachment(0, MARGIN);
		int offset = parent.getBounds().height
				- tabDesc
						.computeSize(SWT.DEFAULT, tabDesc.getLineHeight() * 10).y;
		...
		data = new FormData();
		data.left = new FormAttachment(null, tabList.computeSize(SWT.DEFAULT,
				SWT.DEFAULT).x + 50);
		data.top = new FormAttachment(toolBar, MARGIN);
		data.bottom = new FormAttachment(100, -MARGIN);
		...
		hSash.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				...
				event.y = Math.min(
						Math.max(event.y, tabList.getLocation().y + 60),
						rect.height - 60);
				if (event.detail != SWT.DRAG) {
				...
				if (background.getBgColor1() != null) {
					if (background.getBgColor2() != null) { // gradient
						pattern = new Pattern(device, 0, 0, rect.width,
								rect.height, background.getBgColor1(),
	...
	static Image createThumbnail(Device device, String name) {
		...
		if (src.width != 16 || src.height != 16) {
			...
			gc.drawImage(image, src.x, src.y, src.width, src.height, dest.x,
	 ...
	 *            - The Device
	 * @param color1
	 *            - The first color used to create the image
	 * @param color2
	 *            - The second color used to create the image
	 * 
	 * */
	static Image createImage(Device device, Color color1, Color color2,
	 ...
	 *            - The Device
	 * @param color
	 *            - The color used to create the image
	 * 
	 * */
	...
	void createTabList(Composite parent) {
		...
		Arrays.sort(tabs, new Comparator() {
			public int compare(Object tab0, Object tab1) {
				return ((GraphicsTab) tab0).getText().compareTo(
						((GraphicsTab) tab1).getText());
			}
		});
	...
	void createTabDesc(Composite parent) {
		tabDesc = new Text(parent, SWT.MULTI | SWT.V_SCROLL | SWT.H_SCROLL
				| SWT.WRAP | SWT.BORDER);
		tabDesc.setEditable(false);
		tabDesc.setBackground(Display.getCurrent().getSystemColor(
				SWT.COLOR_WHITE));
	}
	...
	GraphicsTab[] createTabs() {
		return new GraphicsTab[] { new LineTab(this), new StarPolyTab(this),
				...
				new AlphaTab(this), new BallTab(this), new CountDownTab(this),
				new CurvesSWTTab(this), new CurvesTab(this),
				new CustomFontTab(this), new FontBounceTab(this),
				new GradientTab(this), new ImageTransformTab(this),
				new ShapesTab(this), new MazeTab(this), new RGBTab(this),
				new SpiralTab(this), new CardsTab(this), new LineCapTab(this),
				new InterpolationTab(this), new PathClippingTab(this),
				new PathClippingAnimTab(this), new LineStyleTab(this),
				new LineJoinTab(this), new RegionClippingTab(this),
				new CustomAlphaTab(this), new TextAntialiasTab(this),
				new GraphicAntialiasTab(this), new ImageFlipTab(this),
				new ImageScaleTab(this), new PathTab(this), };
	}
	...
	static Image loadImage(Device device, Class clazz, String string) {
		...
		if(pkg!=null)
		{
			prefix=pkg.getName().replace('.', '/');//"org/eclipse/swt/examples/graphics/"
			if(prefix.length()>0)
				prefix+="/";
		}
		
		InputStream stream = /* clazz */Thread
				.currentThread()
